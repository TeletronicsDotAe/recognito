...
import com.bitsinharmony.recognito.algorithms.MFCC;
...
// I have absolutely no idea if the MFCC class has a correct MFCC implementation, or if it is used correctly from here!!!!
public class MfccFeaturesExtractor
    ...
    public MfccFeaturesExtractor(float sampleRate, int poles) {
        super(sampleRate);

        int nnumberofFilters = 24;
        int nlifteringCoefficient = 22;
        boolean oisLifteringEnabled = false; // true
        boolean oisZeroThCepstralCoefficientCalculated = false;
        int nFFTLength = 512;

        this.poles = (oisZeroThCepstralCoefficientCalculated)?poles+1:poles;
        this.windowFunction = new HammingWindowFunction(windowSize);

        mfcc = new MFCC(this.poles,
                sampleRate,
                nnumberofFilters,
                nFFTLength,
                oisLifteringEnabled,
    ...
    public double[] extractFeatures(double[] voiceSample) {
        ...
        for (int i = 0; (i + windowSize) <= voiceSample.length; i += halfWindowLength) {
            ...
            double[] lpcCoeffs = mfcc.getParameters(audioWindow);
